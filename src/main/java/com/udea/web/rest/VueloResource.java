...
public class VueloResource {
     ...
     * @return the {@link ResponseEntity} with status {@code 201 (Created)} and with body the new vuelo, or with status {@code 400 (Bad Request)} if the vuelo already has an ID.
    ...
    public ResponseEntity<Vuelo> partialUpdateVuelo(
        ...
        LOG.debug("REST request to partially update Vuelo : {}, {}", id, vuelo);
        if (vuelo.getId() == null) {
            throw new BadRequestAlertException("Invalid id", ENTITY_NAME, "idnull");
        }
        if (!Objects.equals(id, vuelo.getId())) {
